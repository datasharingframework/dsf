...
certificate_require_expr="./conf/extra/certificate_require_expr.conf"

if [ -e "$certificate_require_expr" ]; then
  echo "Info: Not creating $certificate_require_expr, file exists"
elif [ "$SSL_VERIFY_CLIENT" == "optional" ]; then
  echo "Require expr \"%{SSL_CLIENT_VERIFY} == 'NONE' || %{SSL_CLIENT_S_DN_C} in { $SSL_EXPECTED_CLIENT_S_DN_C_VALUES } && %{SSL_CLIENT_I_DN_CN} in { $SSL_EXPECTED_CLIENT_I_DN_CN_VALUES }\"" > "$certificate_require_expr"
elif [ "$SSL_VERIFY_CLIENT" == "require" ]; then
  echo "Require expr \"%{SSL_CLIENT_S_DN_C} in { $SSL_EXPECTED_CLIENT_S_DN_C_VALUES } && %{SSL_CLIENT_I_DN_CN} in { $SSL_EXPECTED_CLIENT_I_DN_CN_VALUES }\"" > "$certificate_require_expr"
fi
