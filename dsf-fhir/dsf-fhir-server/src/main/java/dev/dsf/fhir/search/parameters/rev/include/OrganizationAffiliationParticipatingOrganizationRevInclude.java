...
public class OrganizationAffiliationParticipatingOrganizationRevInclude extends AbstractRevIncludeParameter
{
	...
	{
		return "(SELECT jsonb_agg(organization_affiliation) FROM current_organization_affiliations WHERE organization_affiliation->'participatingOrganization' @> ('{\"reference\": \"Organization/' || (organization->>'id') || '\"}')::jsonb) AS organization_affiliations";
	}
