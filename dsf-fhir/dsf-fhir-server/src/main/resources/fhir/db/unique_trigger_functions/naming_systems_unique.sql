...
BEGIN
	...
	IF EXISTS (SELECT 1 FROM current_naming_systems WHERE naming_system_id <> NEW.naming_system_id
		AND (
			naming_system->>'name' = NEW.naming_system->>'name'
			OR (
				jsonb_path_exists(NEW.naming_system, '$.uniqueId[*] ? (@.type == "other").value')
				AND
				jsonb_path_query_array(naming_system, '$.uniqueId[*] ? (@.type == "other").value') @>
				jsonb_path_query_array(NEW.naming_system, '$.uniqueId[*] ? (@.type == "other").value')
			) OR (
				jsonb_path_exists(naming_system, '$.uniqueId[*] ? (@.type == "other").value')
				AND
				jsonb_path_query_array(NEW.naming_system, '$.uniqueId[*] ? (@.type == "other").value') @>
				jsonb_path_query_array(naming_system, '$.uniqueId[*] ? (@.type == "other").value')
			)
