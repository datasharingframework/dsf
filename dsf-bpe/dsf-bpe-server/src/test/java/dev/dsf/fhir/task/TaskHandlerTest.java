...
public class TaskHandlerTest
{
	...
	@Spy
	private TaskHelper taskHelper = new TaskHelperImpl();
	...
	public void testCreateBusinessKey()
	{
		...
		Mockito.when(processInstanceQuery
				.variableValueEquals(Mockito.eq(BPMN_EXECUTION_VARIABLE_ALTERNATIVE_BUSINESS_KEY), Mockito.anyString()))
				.thenReturn(processInstanceQuery);
		...
		assertEquals(0, taskBeforeUpdate
				.getInput().stream().filter(
						Objects::nonNull)
				.flatMap(i -> i.getType().getCoding().stream().filter(c -> CODESYSTEM_DSF_BPMN.equals(c.getSystem())
						&& CODESYSTEM_DSF_BPMN_VALUE_BUSINESS_KEY.equals(c.getCode())))
				.count());

		taskHandler.onResource(taskBeforeUpdate);
		Mockito.verify(webserviceClient).update(taskAfterUpdate.capture());

		assertEquals(1, taskAfterUpdate
